  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (res.ok) {
          // show only products uploaded by this admin if seller is present
          const adminEmail = localStorage.getItem('adminEmail') || null;
          const visible = adminEmail ? data.products.filter(p => String(p.seller) === String(adminEmail)) : data.products || [];
          setProducts(visible || []);
        }
      } catch (err) {
